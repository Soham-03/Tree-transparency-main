...
// import React, { useState, useEffect } from 'react';
// import { useRouter } from 'next/router';
// import { Plant, Sprout, AlertTriangle, Leaf, ChevronDown, ChevronUp, BarChart2, 
//          Droplets, RefreshCw, Maximize2, ArrowUp, ArrowDown, Info } from 'lucide-react';
...
//     setRefreshing(true);
//     try {
//       const response = await fetch(`/api/nodeData/${id}`);
//       const jsonData = await response.json();
//       setData(jsonData);
//     } catch (error) {
//       console.error("Error fetching data:", error);
...
//   const getGrowthTrend = () => {
//     if (!data || !data.node_values || data.node_values.length < 2) return 0;
//     const latest = data.node_values[data.node_values.length - 1].height;
//     const previous = data.node_values[data.node_values.length - 2].height;
//     return latest - previous;
//   };

//   const getGrowthRate = () => {
//     if (!data || !data.node_values || data.node_values.length < 2) return 0;
//     const values = data.node_values;
//     const totalGrowth = values[values.length - 1].height - values[0].height;
//     const timeStart = new Date(values[0].timestamp);
//     const timeEnd = new Date(values[values.length - 1].timestamp);
//     const days = (timeEnd - timeStart) / (1000 * 60 * 60 * 24);
//     return (totalGrowth / Math.max(days, 1)).toFixed(2);
//   };

//   const getChartData = () => {
//     if (!data || !data.node_values) return [];
//     return data.node_values.map((entry, index) => ({
//       date: new Date(entry.timestamp).toLocaleDateString(),
//       height: entry.height,
//       status: index === 0 ? "Initial" : 
//               entry.height - data.node_values[index - 1].height > 10 ? "Needs Care" : "Growing"
//     }));
//   };
...
//   if (!data || !data.node_values || data.node_values.length === 0) {
//     return (
//       <div className="flex items-center gap-2 justify-center min-h-screen bg-gradient-to-br from-red-50 to-orange-50 text-red-600">
//         <AlertTriangle size={24} />
//         <h2 className="text-xl">No sensor data available for this plant</h2>
...
//   const latestReading = data.latest_record;
...
//             <h1 className="text-2xl font-bold flex items-center gap-2 text-gray-800">
//               <Sprout className="h-8 w-8 text-green-600" />
//               Plant Growth Monitoring - {data.node_data.nodename}
//             </h1>
//             <p className="text-gray-600 mt-2">
//               Location: {data.node_data.latitude}, {data.node_data.longitude}
//             </p>
//           </div>

//           {/* Main Metrics Cards */}
//           <div className="grid gap-6 mb-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
//             <Card className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 hover:from-green-100 hover:to-emerald-100">
//               <div className="flex justify-between items-center mb-4">
//                 <h3 className="text-gray-600">Current Height
//                   <InfoTooltip text="The latest measured height of your plant" />
//                 </h3>
...
//                 }`}>
//                   {getGrowthTrend() > 0 ? <ArrowUp size={20} /> : <ArrowDown size={20} />}
//                   <span className="font-semibold">{Math.abs(getGrowthTrend())} cm</span>
//                 </div>
//               </div>
//               <div className="text-3xl font-bold mb-2">{latestReading.height} cm</div>
//               <div className="text-sm text-gray-500">
//                 Last updated: {new Date(latestReading.timestamp).toLocaleString()}
//               </div>
//             </Card>

//             <Card className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100">
//               <div className="flex justify-between items-center mb-4">
//                 <h3 className="text-gray-600">Growth Rate
//                   <InfoTooltip text="Average daily growth rate" />
//                 </h3>
//                 <Sprout className="h-6 w-6 text-green-500" />
//               </div>
//               <div className="text-3xl font-bold mb-2">{getGrowthRate()} cm/day</div>
//               <div className="text-sm text-gray-500">Based on {data.node_values.length} readings</div>
//             </Card>

//             <Card className="p-6 bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100">
//               <div className="flex justify-between items-center mb-4">
//                 <h3 className="text-gray-600">Total Readings
//                   <InfoTooltip text="Number of measurements taken" />
//                 </h3>
//                 <BarChart2 className="h-6 w-6 text-blue-500" />
//               </div>
//               <div className="text-3xl font-bold mb-2">{data.node_values.length}</div>
//               <div className="text-sm text-gray-500">Continuous monitoring active</div>
//             </Card>
//           </div>
...
//                     {data.node_values.map((reading, index) => {
//                       const heightChange = index > 0 
//                         ? reading.height - data.node_values[index - 1].height
//                         : 0;
